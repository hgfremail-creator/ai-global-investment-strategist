...
| 6 ✅ | Risk engine (`engine/risk.ts`): synthetic portfolio return series → annualised vol (empirical blended with a ρ=0.4 correlation-floor estimate so demo data stays realistic), max drawdown, Sharpe/Sortino, HHI + effective names, factor look-through, valuation/liquidity/geopolitical gauges; `engine/stress.ts`: 5 deterministic factor-shock scenarios + custom; `services/risk.ts` persists `RiskMetric` + `StressTest`; Risk dashboard page | ✅ 42 tests (bounded metrics, exposure sums, TW→geo risk, AI-crash vs cash, gold in geo scenario), build green |
| 7 ✅ | Weekly strategy engine: `weeklyReview.ts` (movable demo clock — +1 week/version, real "now" for live providers), full refresh pipeline (ingest→analysis→sleeves→portfolio→risk→recs→report), `weeklyReport.ts` (per-`StrategyChange` evidence from score deltas + regime delta, 15-section `ResearchReport`), incumbency bonus in `allocation.ts` (turnover: v1→v4 removed just 2 names), `/api/weekly/run` + cron route, Weekly Review page (change table + rendered report), `react-markdown` | ✅ 43 tests, build green, `docs/WEEKLY_REFRESH.md` |
| 8 ✅ | Historical tracking: `engine/diff.ts` (pure `computeChanges`), `getVersionComparison` / `getSleeveHistory`; History page (immutable version list + stacked sleeve-history area chart + compare selector), `/history/[version]` snapshot, `/history/compare?a=&b=` (sleeve/position/factor deltas + recorded reasons); `scripts/verify-immutability.ts` (`npm run verify:history`) confirms prior versions byte-stable after a weekly run | ✅ 46 tests, build green, immutability check passes |
| 9 ✅ | `services/advisor.ts` — LLM path (grounded prompt, text guardrail, `AIAnalysis` audit) + deterministic intent router (why-own / why-reduce / X-vs-Y / biggest-risk / best-risk-reward / market-drop / summary), every answer cites stored data; `services/whatIf.ts` — non-persisting recompute for capital / risk / horizon / excluded sectors / min-gold + custom shock, diffed vs the live strategy; Advisor chat page + What-If page (nav item added); `runCustomScenario` in stress engine | ✅ 53 tests (router intents, no guaranteed-return phrasing), build green, live API probe passes |
| 10 ✅ | `engine/performance.ts` + `services/performance.ts`: chains each strategy version's allocation over the period it was live into a paper NAV series vs MSCI World + a risk-profile-blended benchmark; `PerformanceChart` + `DrawdownChart` (underwater) on the Dashboard with total / weekly return, max drawdown, vol, Sharpe/Sortino; a11y pass (skip link, `aria-current`, focus-visible rings, chart `role="img"` + labels) | ✅ 57 tests (NAV chaining, benchmark, missing-data resilience), build green |
| 11 ✅ | Spec §47 sweep: 60 unit tests + a 9-test integration suite (`vitest.integration.config.ts` → temp SQLite DB via `globalSetup`/`setup`, real seed + ingest + `generateStrategy` + `runWeeklyReview`) covering 100%-allocation, risk/horizon/capital recompute, weekly-change identification + immutability, source attachment, no-rec-without-evidence, determinism, currency; `scripts/verify-no-secrets.mjs` + `src/test/security-source.test.ts` (client bundle / source hygiene); `seed-core.ts` refactor; `docs/SECURITY_CHECKLIST.md` + `docs/COMPLIANCE_CHECKLIST.md` | ✅ all green |
| 12 ✅ | `src/lib/db.ts` PostgreSQL branch (`@prisma/adapter-pg`, engine-free, activated by `DATABASE_PROVIDER=postgresql`); CSP + full security headers in `next.config.ts`; `src/lib/rateLimit.ts` token-bucket on `/api/advisor` `/api/whatif` `/api/weekly/run` `/api/auth/login`; `.github/workflows/ci.yml` (typecheck·lint·unit·integration·verify·build + a Postgres integration job); `docs/METHODOLOGY.md` + `docs/API.md` complete the §51 doc set | ✅ lint clean, 60 unit + 9 integration green, build + verify-no-secrets green |
