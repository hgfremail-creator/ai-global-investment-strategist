...
| 2 | Data layer: provider interfaces, DemoProvider (deterministic), Source records, ingestion service, Markets page | provider contract tests, ingestion idempotency |
| 3 | Analysis layer: indicators, scoring model, regime engine, Research page score breakdown | scoring determinism, weights sum, regime bands |
| 4 | Portfolio engine: sleeve matrix, tilts, selection, constraint projection, FX; Strategy + Portfolio pages | total=100%, constraints, monotonic vs risk, FX |
| 5 | AI layer: Anthropic structured client + Zod + fallback writer + audit; "Why / Why not", Committee, Devil's Advocate | schema validation, fabricated‑source rejection, fallback path |
